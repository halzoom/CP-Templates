...
public:
    ...
    void setTestCase(int testCase) {
        if (testCase < 0 || testCase > __TESTLIB_MAX_TEST_CASE)
            __testlib_fail(
                    format("testCase expected fit in [1,%d], but %d doesn't", __TESTLIB_MAX_TEST_CASE, testCase));
...
public:
    ...
    std::string testset() const {
        if (!_initialized)
            __testlib_fail(
                    "Validator should be initialized with registerValidation(argc, argv) instead of registerValidation() to support validator.testset()");
        return _testset;
    }
    ...
    std::string group() const {
        if (!_initialized)
            __testlib_fail(
                    "Validator should be initialized with registerValidation(argc, argv) instead of registerValidation() to support validator.group()");
        return _group;
    }
        ...
        if (!_testMarkupFileName.empty()) {
            ...
            if (!readChars.empty()) {
                ...
                if (_testMarkupFileName == "stdout")
                    ...
                    if (NULL == f)
                        __testlib_fail(
                                "Validator::writeTestMarkup: can't write test markup to (" + _testMarkupFileName + ")");
                }
                ...
                if (!standard_file)
                    if (std::fclose(f))
                        __testlib_fail(
                                "Validator::writeTestMarkup: can't close test markup file (" + _testCaseFileName + ")");
            }
        }
    }
            ...
            if (!readChars.empty()) {
                ...
                if (!testCaseContent.empty()) {
                    ...
                    if (_testCaseFileName.empty() || _testCaseFileName == "stdout")
                        ...
                        if (NULL == f)
                            __testlib_fail(
                                    "Validator::writeTestCase: can't write test case to (" + _testCaseFileName + ")");
                    }
                    ...
                    if (!standard_file)
                        if (std::fclose(f))
                            __testlib_fail(
                                    "Validator::writeTestCase: can't close test case file (" + _testCaseFileName + ")");
                }
            }
        }
    }
...
struct TestlibFinalizeGuard {
    ...
    ~TestlibFinalizeGuard() {
        ...
        if (_alive) {
            ...
            if (!registered)
                __testlib_fail(
                        "Call register-function in the first line of the main (registerTestlibCmd or other similar)");
        }
...
InStream::readUnsignedLong(unsigned long long minv, unsigned long long maxv, const std::string &variableName) {
    ...
    if (result < minv || result > maxv) {
        if (readManyIteration == NO_INDEX) {
            if (variableName.empty())
                quit(_wa,
                     ("Unsigned integer " + vtos(result) + " violates the range [" + toHumanReadableString(minv) +
                      ", " + toHumanReadableString(maxv) +
                      "]").c_str());
            else
                quit(_wa,
            ...
            if (variableName.empty())
                ...
                quit(_wa, ("Unsigned integer element " + std::string(variableName) + "[" + vtos(readManyIteration) +
                           "] equals to " + vtos(result) + ", violates the range [" + toHumanReadableString(minv) +
                           ", " + toHumanReadableString(maxv) +
                           "]").c_str());
        }
    }
...
NORETURN void __testlib_help() {
    ...
    std::fprintf(stderr,
                 "    [--testset testset] [--group group] <input-file> <output-file> <answer-file> [<report-file> [<-appes>]]\n\n");
...
void setAppesModeEncoding(std::string appesModeEncoding) {
    static const char *const ENCODINGS[] = {"ascii", "utf-7", "utf-8", "utf-16", "utf-16le", "utf-16be", "utf-32",
                                            ...
                                            "windows-1255",
                                            "windows-1256", "windows-1257", "windows-1258", "gb2312", "gbk", "gb18030",
                                            "big5", "shift-jis", "euc-jp", "euc-kr",
                                            "euc-cn", "euc-tw", "koi8-r", "koi8-u", "tis-620", "ibm437", "ibm850",
                                            "ibm852", "ibm855", "ibm857",
                                            "ibm860", "ibm861", "ibm862", "ibm863", "ibm865", "ibm866", "ibm869",
                                            "macroman", "maccentraleurope", "maciceland",
                                            "maccroatian", "macromania", "maccyrillic", "macukraine", "macgreek",
                                            "macturkish", "machebrew", "macarabic", "macthai", "hz-gb-2312",
                                            "iso-2022-jp", "iso-2022-kr", "iso-2022-cn", "armscii-8", "tscii", "iscii",
                                            "viscii", "geostd8", "cp949", "cp874",
                                            "cp1006", "cp775", "cp858", "cp737", "cp853", "cp856", "cp922", "cp1046",
                                            "cp1125", "cp1131",
                                            "ptcp154", "koi8-t", "koi8-ru", "mulelao-1", "cp1133", "iso-ir-166", "tcvn",
                                            "iso-ir-14", "iso-ir-87", "iso-ir-159"};
